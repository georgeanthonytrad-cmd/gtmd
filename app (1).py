...
# Optional API keys. App still works for many indicators without keys using yfinance.
...
FMP_LEGACY = "https://financialmodelingprep.com/api/v3"
...
def fmp_get(endpoint, params=None):
    ...
    try:
        r = requests.get(f"{FMP_LEGACY}/{endpoint}", params=params, timeout=15)
...
def get_options_summary(symbol):
    result = {
        ...
        "options_status": "Tradier key missing or data unavailable",
    }
    ...
    try:
        ...
        dates = expirations.get("expirations", {}).get("date", [])
        if not dates:
            return result
        ...
        result["options_status"] = "Options data loaded from Tradier"

    except Exception as e:
        result["options_status"] = f"Options unavailable: {e}"
...
def build_metrics(symbol, tsp_yes, sr_yes):
    yf_info, prices = get_yf_data(symbol)

    fmp_quote = get_fmp_quote(symbol)
    fmp_ratios = get_fmp_ratios(symbol)
    fmp_growth = get_fmp_growth(symbol)
    fmp_target = get_fmp_analyst_target(symbol)
    ...
    ttm_pe = safe_float(fmp_quote.get("pe"), safe_float(yf_info.get("trailingPE")))
    forward_pe = safe_float(yf_info.get("forwardPE"))
    peg = safe_float(yf_info.get("pegRatio"), safe_float(fmp_ratios.get("priceEarningsToGrowthRatioTTM")))

    eps_raw = safe_float(fmp_growth.get("growthEPS"))
    rev_raw = safe_float(fmp_growth.get("growthRevenue"))

    # Fallback from yfinance if FMP growth missing
    ...
    avg_target = safe_float(
        fmp_target.get("priceTargetAverage"),
    ...
    try:
        ...
        if earnings_date == "N/A" and cal is not None and not cal.empty:
            for possible in ["Earnings Date", "Earnings Average"]:
                if possible in cal.index:
                    raw_date = cal.loc[possible].iloc[0]
                    earnings_date = str(pd.to_datetime(raw_date).date())
                    break
    except Exception:
        pass
...
if run and symbol:
    ...
    if np.isnan(safe_float(values["Current Price"])):
        st.error("No price data loaded. Check ticker spelling, internet access, or API limits.")
    else:
        st.success("Price and technical indicators loaded.")

    if "unavailable" in str(values.get("Options Data Status", "")).lower() or "missing" in str(values.get("Options Data Status", "")).lower():
