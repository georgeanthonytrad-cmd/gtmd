...
def fmp_get(path, params=None):
    ...
    try:
        ...
        if r.status_code != 200:
            st.sidebar.warning(f"FMP error {r.status_code} on {path}")
            return None
        return r.json()
...
def get_options_summary(symbol):
    result = {
        ...
        "options_status": "Tradier key missing or options data unavailable",
    }
    if not TRADIER_TOKEN:
        return result
...
def build_metrics(symbol, tsp_yes, sr_yes):
    ...
    # Fallback to Yahoo only if FMP does not load prices.
...
if run and symbol:
    ...
    if np.isnan(safe_float(values["Current Price"])):
        ...
        st.info("Go to Streamlit → App settings → Secrets, then add: FMP_API_KEY = \"your_key\"")
    else:
